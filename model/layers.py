...
class Layer(object):
    def __init__(self, incoming, name=None, variable_reuse=None, weight_normalization=False, **kwargs):
        ...
        print ('in layer: ', self.input_shape, isinstance(incoming, tuple) )
    ...
    def output_shape(self):
        ...
        print ('in get output shape', self.input_shape, shape)
...
class BaseConvLayer(Layer):
    def __init__(self, incoming, num_filters, filter_size, stride=1, pad="VALID",
        ...
        """
        print ('in base_conv: ', incoming.input_shape)
    ...
    def get_output_shape_for(self, input_shape):
        print ('base conv get output shape for', input_shape)
        if self.pad == 'SAME':
            pad = ('same',) * self.n
        elif self.pad == 'VALID':
            pad = (0,) * self.n
        ...
        print ('batchsize ', batchsize, input_shape)

    
        for input, filter, stride, p in zip(input_shape[1:2], self.filter_size,self.stride, pad):
            a = conv_output_length(input, filter, stride, p)
            print (input, filter, stride, p, a)

        return ((batchsize,) +
                tuple(conv_output_length(input, filter, stride, p)
                      for input, filter, stride, p
                      in zip(input_shape[1:2], self.filter_size,
                             self.stride, pad))) + (self.num_filters,)

    def get_output_for(self, input, **kwargs):
        print ('base conv', input)
        ...
        if self.b is None:
            ...
            activation = conved + tf.reshape(self.b, (1, 1, self.num_filters))

        print(activation, self.untie_biases)
        return self.nonlinearity(activation)
...
class Conv2DLayer(BaseConvLayer):
    def __init__(self, incoming, num_filters, filter_size, stride=(1, 1),
        ...
        print('out conv2d', self.output_shape)
...
class Conv1DLayer(BaseConvLayer):
    def __init__(self, incoming, num_filters, filter_size, stride=(1, 1),
        ...
        super(Conv1DLayer, self).__init__(incoming=incoming, num_filters=num_filters, filter_size=filter_size,
                                          stride=1, pad=pad, untie_biases=untie_biases, W=W, b=b,
                                          nonlinearity=nonlinearity, n=1, **kwargs)
        self.convolution = convolution
        print('out convd', self.output_shape)

    def convolve(self, input, **kwargs):
        print ('W: ', self.W)
        conved = self.convolution(input, self.W, stride=1, padding=self.pad)
        # conved = self.convolution(input, filters=self.num_filters, kernel_size = self.filter_size[0], strides=1, padding=self.pad)
        return conved
...
def get_output(layer_or_layers, inputs=None, **kwargs):
    ...
    print ('all layer num: ', len(all_layers))
    for layer in all_layers:
        if layer not in all_outputs:
            ...
            print('all_outputs[layer] ', all_outputs[layer], layer)
            try:
